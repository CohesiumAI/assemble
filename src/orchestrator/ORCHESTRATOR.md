...
6. **Execute sequentially** — For each step:
   ...
   f. **Gate Keeper check** — Before any step that matches ALL of these conditions:
      ...
      Doom produces `doom-verdict.md`. If REJECTED, halt and report to user.
...
2. **Suggest escalation** — Tell the user: "Deadpool has flagged critical concerns. Would you like Doctor Doom's strategic verdict? (invoke @doctor-doom)"
...
3. Doom produces `doom-verdict.md` with APPROVED / APPROVED WITH CONDITIONS / REJECTED verdict
4. If REJECTED → workflow halts, user is informed with blocking issues
5. If APPROVED WITH CONDITIONS → mitigations are injected into the next step's context
6. If APPROVED → workflow proceeds normally
