...
## Key Decisions
<!-- Important decisions with rationale. Format: [date] decision — rationale. -->
...
function doctor(projectDir, fixMode) {
  ...
  // ─── 10b. _memory.md format check (new format with Project Summary + Recurring Issues) ──
  check('Cross-session memory format', () => {
    if (!memory) return true;
    const memoryFile = path.join(resolvedOutputDir, '_memory.md');
    if (!fs.existsSync(memoryFile)) return true; // check 10 handles existence
