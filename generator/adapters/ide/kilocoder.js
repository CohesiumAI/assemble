...
module.exports = {
  ...
  getOutputPaths(projectDir, { agents = [], skills = {}, workflows = [] } = {}) {
    ...
    paths.push(path.join(projectDir, '.kilocoder', 'commands.md'));
    paths.push(path.join(projectDir, '.kilocoder', 'orchestrator.md'));
    return paths;
  },
  ...
  generate(projectDir, { agents = [], skills = {}, workflows = [], commands, orchestrator, config = {} }) {
    ...
    fs.writeFileSync(path.join(projectDir, '.kilocoder', 'commands.md'), renderCommandRegistry(agents, skills, workflows, config.governance, config.yolo), 'utf-8');
    if (orchestrator) {
      fs.writeFileSync(path.join(projectDir, '.kilocoder', 'orchestrator.md'), renderOrchestrator(orchestrator, config), 'utf-8');
    }
  },
  ...
  validate(projectDir) {
    ...
    for (const rootFile of ['commands.md', 'orchestrator.md']) {
      const p = path.join(projectDir, '.kilocoder', rootFile);
      if (fs.existsSync(p) && fs.readFileSync(p, 'utf-8').trim().length === 0) errors.push(`Empty: .kilocoder/${rootFile}`);
    }
