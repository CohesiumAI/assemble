 ...
 * Generates .codebuddy/agents/{agent-name}.md files
 */
...
module.exports = {
  ...
  getOutputPaths(projectDir, { agents = [] } = {}) {
    return agents.map(agent => {
      ...
      return path.join(projectDir, '.codebuddy', 'agents', `${slug}.md`);
    });
  },

  generate(projectDir, { agents = [], skills = {}, workflows = [], commands, orchestrator, config = {} }) {
    const agentsDir = path.join(projectDir, '.codebuddy', 'agents');
